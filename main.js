...
const renderBooks = ({ numFound, docs }) => {
   ...
   docs.forEach(
      ({ title, cover_i, author_name, first_publish_year, publisher }, i) => {
        ...
        <div class="card border rounded text-center shadow-lg" style="min-height: 100%;" id="card">
            <img src="${imageUrl}" class="card-img-top img-fluid" alt="Book Cover" style="height: 20rem;"> 
                ...
                <div class="text-start px-2">
                    <p class="text-light "><span class="fw-bold text-color">Author: </span> ${
                       author_name ? author_name : 'No Author Name Found'
                    }</p>
                    <p class="text-light "><span class="fw-bold text-color">First Published: </span> ${
                       first_publish_year
                          ? first_publish_year
                          : 'No Publish Year Found'
                    }</p>
                    <p class="text-light "><span class="fw-bold text-color">Publisher: </span> ${
                       publisher ? publisher.slice(0, 2) : 'No Publisher Found'
                    }</p>
                </div>
        </div>
